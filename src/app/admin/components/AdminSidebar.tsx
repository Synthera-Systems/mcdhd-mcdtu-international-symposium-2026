...
export default function AdminSidebar({ activeTab, setActiveTab, handleLogout, isOpen, onClose }: SidebarProps) {
  const tabs = [
    ...
    { id: "CONFIRMED", label: "Confirmed", icon: "M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" },
  ...
  return (
    <>
      ...
      <aside 
        className={`fixed inset-y-0 left-0 w-64 px-3 bg-white border-r border-surface-dim/30 z-50 flex flex-col shadow-[4px_0_24px_rgba(0,0,0,0.02)] transition-transform duration-300 ease-in-out md:translate-x-0 ${
      ...
      >
        <div className="p-8 pb-6 flex items-center justify-between">
          ...
          <button onClick={onClose} className="md:hidden p-1 rounded-lg text-on-surface-variant hover:bg-surface-bright">
        ...
        <nav className="flex-1 px-5 space-y-2 mt-2">
          {tabs.map((tab) => (
            <button 
              ...
              className={`w-full flex items-center gap-4 px-4 py-3 rounded-xl text-sm font-medium transition-all cursor-pointer whitespace-nowrap ${
                activeTab === tab.id 
        ...
        <div className="p-6 space-y-4 shrink-0">
          <button 
            onClick={() => handleTabClick("EXPORT-DATA")} 
